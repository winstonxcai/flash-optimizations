#!/usr/bin/env bash
# TopMag50 rerun of the 8 HARD Sangfor tasks at --context-length 262144 (local server
# on 10.72.1.175:30211, GPUs 4-7). Purpose: does removing the sglang 103k effective
# input cap (135168 - 32000 completion) close the cloud-vs-local crater for TopMag50?
# Same 8 tasks + same harness as run_hard_native.sh; only server window differs.
set -u
cd /data/zyj/YJYBench
STAMP=20260831
MASTER=/data/zyj/YJYBench/results/dsv4-topmag50-hard8-262k_master.log
...
while IFS='|' read -r INST DIFF LANG NATIVE; do
  [ -z "$INST" ] && continue
  RID="dsv4-topmag50-hard8-262k-$(echo "$INST" | sed 's/[^a-z0-9-]/_/g')_$STAMP"
