#!/usr/bin/env bash
# NATIVE-CSA baseline rerun of the 8 HARD Sangfor tasks at --context-length 262144
# (local server 10.72.1.175:30212, GPUs 0-3). Parallel leg to the TopMag50@262144
# run on 30211/4-7. Same 8 tasks, same harness; only server window differs from the
# 135168 cratered runs. Compare both 262k legs vs cloud-native task_20260825_195126_744.
set -u
...
while IFS='|' read -r INST DIFF LANG NATIVE; do
  [ -z "$INST" ] && continue
  RID="dsv4-hardnative-262k-$(echo "$INST" | sed 's/[^a-z0-9-]/_/g')_$STAMP"
