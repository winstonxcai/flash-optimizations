#!/usr/bin/env bash
# TopMag50 (latent) rerun of the easy+medium Sangfor tasks at --context-length 262144.
# Server: 10.72.1.175:30211 (GPUs 4-7, mustafar TopMag package, SGLANG_OPT_TOPMAG=1).
# ORDER: mediums that hit the 135k context-400s first (the confounded set), then the
# other mediums, then easy. 17 tasks. RID dsv4-topmag50-em-262k-<task>_20260901.
set -u
...
while IFS='|' read -r INST DIFF LANG NATIVE; do
  [ -z "$INST" ] && continue
  [[ "$INST" == \#* ]] && continue
  RID="dsv4-topmag50-em-262k-$(echo "$INST" | sed 's/[^a-z0-9-]/_/g')_$STAMP"
