...
# on GPUs 4-7. Runs INSIDE ruler-eval. Port 30211 (docker_env_config_dsv4-windowed
# points there). Stage-1 packed = SGLANG_OPT_TOPMAG=1 XKV_TOPMAG_KEEP=0.5
# SGLANG_OPT_TOPMAG_PACKED_C4=1 (328-byte rows, no native shadow pool).
# Requires `python3 -m mustafar patch` applied (verified). MASTER_PORT 29624
# distinct from server B's 29623.
set -u
KEEP=${XKV_TOPMAG_KEEP:-0.5}
LOG=/mnt/host_root/home/jovyan/winstonxcai/flash-optimizations/mustafar/logs/serve_packedA_262k.log
...
nohup python3 -m sglang.launch_server \
  ...
  > "$LOG" 2>&1 &
echo "launched packedA-262k pid $!"
