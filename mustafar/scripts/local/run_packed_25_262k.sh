#!/usr/bin/env bash
# Stage-1 PACKED TopMag50 eval of 25 NEW Sangfor tasks at --context-length 262144.
# Two TP4 packed servers on 10.72.1.175: A=30211 (GPUs 4-7), B=30212 (GPUs 0-3),
# both SGLANG_OPT_TOPMAG=1 KEEP=0.5 PACKED_C4=1. Tasks pre-assigned alternating
# A/B so the two chains run concurrently (interleaved) and both servers stay busy.
# Order: hard (8) -> medium (10) -> easy (7). Split: 13 cloud-pass / 12 cloud-fail
# (hard 4P/4F, medium 5P/5F, easy 4P/3F), all with >=8 cloud tests, excluding the
# previous 25 topmag50-25d tasks. RID dsv4-packed-262k-<task>_20260901.
set -u
cd /data/zyj/YJYBench
STAMP=20260901
MASTER=/data/zyj/YJYBench/results/dsv4-packed-262k_master.log
...
run_chain() {
  ...
  while IFS='|' read -r INST DIFF LANG CLOUD; do
    [ -z "$INST" ] && continue
    [[ "$INST" == \#* ]] && continue
    RID="dsv4-packed-262k-$(echo "$INST" | sed 's/[^a-z0-9-]/_/g')_$STAMP"
...
echo "PACKED_START $(date '+%F %H:%M:%S')" | tee -a "$MASTER"
