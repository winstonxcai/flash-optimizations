...
- `kernel-run.sh [--validity-only|--speed-only]` — run the mustafar **GPU suites**
  ...
  `MUSTAFAR_LEASE` outside it, then writes `validity.log` and `speed.{json,csv}`
  under `mustafar/results/sparse-<stamp>/`. Never uses `--gpus all`. See its
