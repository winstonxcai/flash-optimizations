"""H100/A800 Stage-1 pack and gather/unpack microbenchmark matrix."""
...
def run(output_dir: str | Path | None = None) -> dict[str, object]:
    ...
    pack_c4_rows(latent, mask, weight, 1.0e-6, plan, locations, buffers)
    ...
    for batch in batch_sizes:
        ...
        fn = lambda: pack_c4_rows(
            latent[:batch], mask[:batch], weight, 1.0e-6,
            local_plan, locations[:batch], buffers,
        )
    ...
    for batch in batch_sizes:
        for selected in selected_counts:
            ...
            bf16_fn = lambda: unpack_gather_c4_bf16(
            ...
            workspace = NativeC4Workspace.allocate(batch, selected, 64, device)
            native_fn = lambda: unpack_gather_c4_native(
    ...
    if output_dir is not None:
        ...
        (output_path / "packed-c4-microbench.json").write_text(
            json.dumps(result, indent=2, sort_keys=True) + "\n"
        )
        with (output_path / "packed-c4-microbench.csv").open("w", newline="") as f:
...
if __name__ == "__main__":
    run(os.environ.get("MUSTAFAR_RESULTS_DIR"))
