...
def _load():
    ...
    try:
        _extension = importlib.import_module("remnant._fused")
    except Exception as exc:
        _load_error = exc
        raise RuntimeError(
            "Fused was requested, but remnant._fused could not be loaded"
        ) from exc
    return _extension
...
def _emit_dispatch_marker(optimized: bool, candidate: str | None = None) -> None:
    ...
    with _marker_lock:
        if marker not in _markers_emitted:
            print(f"REMNANT_FUSED_DISPATCH={marker}", flush=True)
