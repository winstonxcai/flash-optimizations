...
SCRIPT = ROOT / "remnant/scripts/local/bench-serving.sh"


class ShellTests(unittest.TestCase):
    def test_model_revision_and_results_volume_are_not_mode_names(self):
        from remnant.scripts.modal import app
        ...
        self.assertEqual(
            ast.literal_eval(assignment.value.args[0]), "remnant-stage2a-results"
        )
    ...
    def test_recorded_native_pool_field_names_are_preserved(self):
        text = (ROOT / "remnant/tests/fixtures/modal/native-pool.log").read_text()
