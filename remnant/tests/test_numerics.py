...
class ReferenceTests(unittest.TestCase):
    ...
    def test_public_names_and_workspace_layout(self):
        import remnant
        from remnant import config, packed, patching

        self.assertEqual(remnant.PACKED_RECORD_BYTES, 328)
        self.assertEqual(config.PACKED_KEPT_VALUES, 256)
        self.assertEqual(config.NATIVE_RECORD_BYTES, 584)
        self.assertEqual(len(remnant.__all__), len(set(remnant.__all__)))
        for name, module in remnant._LAZY_EXPORTS.items():
            self.assertIs(
                getattr(remnant, name),
                getattr(importlib.import_module(f"remnant.{module}"), name),
            )
        self.assertIs(remnant.patch, patching.patch)
