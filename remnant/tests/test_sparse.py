...
from remnant import config, reference, sparse
...
_BACKEND_ORIGINAL = Path(config.DSV4_BACKEND + ".remnant.orig")
...
class BackendBranchTests(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        from remnant import patching
        from remnant.patches.attention import _backend_edits
    ...
    def _offsets(self):
        ...
        return {
            ...
            "packed": out.index("## REMNANT (decode/small-extend native"),
...
class LoaderTests(unittest.TestCase):
    ...
    def test_available_reflects_an_installed_extension(self):
        self.assertEqual(
            sparse.sparse_available(),
            importlib.util.find_spec("remnant._sparse") is not None,
        )
