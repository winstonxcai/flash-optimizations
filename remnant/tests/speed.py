...
:mod:`remnant.tests.validity`, so a leg name means one thing across both suites:
validity asks "does this leg match native", this asks "what does it cost".
Columns are :data:`remnant.tests.harness.COLUMNS` -- ``native`` first, then the
...
def _write(records: list[dict[str, object]], summary: dict[str, object]) -> None:
    ...
    results_dir = os.environ.get("REMNANT_RESULTS_DIR")
    if not results_dir:
        return
...
def run_store_breakdown(
    *,
    batches: tuple[int, ...] = (1, 8, 15, 21),
    rounds: int = 3,
    samples: int = 500,
) -> dict[str, object]:
    """Measure TopMag, packing, and their combined production store path."""
    if not torch.cuda.is_available():
        raise RuntimeError("store breakdown requires CUDA")
    if not batches or any(batch < 1 for batch in batches):
        raise ValueError("batches must contain positive values")
    if min(rounds, samples) < 1:
        raise ValueError("rounds and samples must be positive")

    device = torch.device("cuda")
    ops = _resolve_ops()
    records: list[dict[str, object]] = []
    modes = ("topmag", "pack", "topmag_pack")
    print(
        f"[store-breakdown] batches={list(batches)} rounds={rounds} "
        f"samples={samples}",
        flush=True,
    )

    for batch in batches:
        # A decode store writes one compressor row per active request. Keeping
        # context_rows=batch makes the row count match the requested batch size.
        workload = harness.Workload(f"store-b{batch}", batch, batch)
        case = harness.build_case(workload, device, seed=20260913)
        with harness.leg_env("packed.bf16"):
            buffers = PackedBuffers(
                torch.zeros(
                    batch, config.PACKED_KEPT_VALUES,
                    dtype=torch.uint8, device=device
                ),
                torch.zeros(
                    batch, config.BITMAP_WORDS,
                    dtype=torch.uint64, device=device
                ),
                torch.zeros(
                    batch, config.PACKED_SCALE_BYTES,
                    dtype=torch.uint8, device=device
                ),
            )

            def topmag() -> None:
                reference.topmag_keep_mask(case.latent, 0.5)

            def pack() -> None:
                ops.pack_rows(
                    case.latent, case.mask, case.weight, 1.0e-6,
                    case.plan, case.locations, buffers,
                )

            def topmag_pack() -> None:
                mask = reference.topmag_keep_mask(case.latent, 0.5)
                ops.pack_rows(
                    case.latent, mask, case.weight, 1.0e-6,
                    case.plan, case.locations, buffers,
                )

            callables = {"topmag": topmag, "pack": pack, "topmag_pack": topmag_pack}
            for round_id in range(1, rounds + 1):
                order = modes[(round_id - 1) % len(modes):] + modes[:(round_id - 1) % len(modes)]
                for mode in order:
                    timing = harness.timed(
                        harness.captured(callables[mode]),
                        warmup=10,
                        repeats=samples,
                    )
                    records.append({
    ...
    for batch in batches:
        ...
        for mode in modes:
            entries = [r for r in records if r["batch"] == batch and r["mode"] == mode]
            by_mode[mode] = {
                key: sum(float(r[key]) for r in entries) / len(entries)
                for key in ("mean_us", "p50_us", "p95_us")
            }
        combined = by_mode["topmag_pack"]["p50_us"]
        by_mode["derived"] = {
            "topmag_share_of_combined": by_mode["topmag"]["p50_us"] / combined,
            "pack_share_of_combined": by_mode["pack"]["p50_us"] / combined,
            "combined_minus_pack_us": combined - by_mode["pack"]["p50_us"],
            "combined_minus_topmag_us": combined - by_mode["topmag"]["p50_us"],
        }
        aggregates[str(batch)] = by_mode

    summary = {
        "suite": "store_breakdown",
        "gpu": torch.cuda.get_device_name(),
        ...
        "note": "topmag_pack generates a fresh mask before each pack_rows call",
    }
    results_dir = os.environ.get("REMNANT_RESULTS_DIR")
    if results_dir:
        path = Path(results_dir)
        path.mkdir(parents=True, exist_ok=True)
        (path / "store-breakdown.json").write_text(
            json.dumps(summary, indent=2, sort_keys=True) + "\n"
        )
        print(f"[store-breakdown] wrote {path / 'store-breakdown.json'}", flush=True)
    return summary
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--store-breakdown",
        action="store_true",
        help="measure TopMag, FP8 packing, and their combined store path",
    )
    parser.add_argument("--store-batches", default="1,8,15,21")
    parser.add_argument("--reconstruction", choices=("timing", "validate", "sanitizer", "profile"))
    args = parser.parse_args()
    if args.store_breakdown:
        run_store_breakdown(
            batches=tuple(int(batch) for batch in args.store_batches.split(",")),
        )
