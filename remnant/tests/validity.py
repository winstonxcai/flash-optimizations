...
(Native, stock 584-B; Packed, ``remnant packed-328``) plus the two CUDA
extensions under test:
  ...
  ``native``       stock SGLang with every ``SGLANG_OPT_TOPMAG*`` flag off:
                   ...
                   ``flash_mla_sparse_fwd``. No remnant code, no Triton. This is
  ...
  ``fused``        ``remnant._fused``, replacing ``packed.native``. Needs
                   ``remnant._fused``.
...
Direct run prints per-case JSON::
    ...
    python3 -m remnant.tests.validity
"""
