...
`SGLANG_OPT_TOPMAG*` flag off, no remnant code, no Triton — and never a
...
docker exec remnant bash -c 'cd /sgl-workspace/sglang-lowrank && \
  ...
  python3 -m unittest remnant.tests.test_numerics -v'
```

Direct, prints per-case JSON (and writes `speed.{json,csv}` when
`REMNANT_RESULTS_DIR` is set):

```sh
python3 -m remnant.tests.validity   # every stage, every available leg
python3 -m remnant.tests.validity --legs sparse       # the sparse leg only
python3 -m remnant.tests.validity --write-baseline    # pin the regression gate
python3 -m remnant.tests.speed      # every stage, every available leg
python3 -m remnant.tests.speed --legs fused,sparse    # a subset of the candidates
```

Package CLI equivalents: `python3 -m remnant selftest` and
`python3 -m remnant packed_selftest`.
