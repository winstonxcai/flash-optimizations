"""Live behaviour of :mod:`remnant.patching` -- offline, no network, no SGLang.
...
Run offline::
    ...
    python3 -m unittest remnant.tests.test_patching -v
"""
...
from remnant import config, patching
...
class _PatchedTree:
    ...
    def __init__(self, stack: ExitStack, fixtures) -> None:
        ...
        stack.enter_context(
            patch.object(config, "PACKAGE_ROOT", "/remnant-regression-root")
        )
        self.output = stack.enter_context(redirect_stdout(io.StringIO()))

    def backup(self, name: str) -> Path:
        return Path(str(self.paths[name]) + ".remnant.orig")
...
class PatchMachineryTests(unittest.TestCase):
    ...
    def test_patch_compile_verify_and_restore(self):
        ...
        patching.verify()
        self.assertEqual(
            self.tree.output.getvalue().count("remnant_markers="),
    ...
    def test_missing_or_duplicate_anchor_does_not_write_target(self):
        ...
        for content in ("untouched\n", "anchor\nanchor\n"):
            ...
            self.assertFalse(Path(str(path) + ".remnant.orig").exists())
...
class RealAnchorTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # pristine base is the .remnant.orig backup when present, else the file
        ...
        for name in TARGET_NAMES:
            ...
            backup = Path(str(source) + ".remnant.orig")
            if backup.exists():
                ...
                if config.MARKER in original:
                    raise unittest.SkipTest(
                        f"{source} is patched with no .remnant.orig backup; "
...
class ImportContractTests(unittest.TestCase):
    """``remnant.patch`` and friends resolve without torch or sglang present."""
    ...
    def test_patch_api_imports_without_torch_or_sglang(self):
        subprocess.run(
            [
                ...
                "-c",
                (
                    "import sys, remnant; from remnant import patching; "
                    "assert remnant.patch is patching.patch; "
                    "assert remnant.unpatch is patching.unpatch; "
                    "assert remnant.verify is patching.verify; "
