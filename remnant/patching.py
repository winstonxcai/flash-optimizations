"""Apply, restore, and verify Remnant patches in the SGLang source tree."""
    ...
    for anchor, new in edits:
        if source.count(anchor) != 1:
            raise AssertionError(
                f"[remnant] anchor count != 1 in {path}: {anchor[:70]!r}"
            )
...
def _plan(*, restoring: bool = False):
    ...
    for filename, factory in _TARGETS:
        ...
        backup = Path(filename + ".remnant.orig")
        if not backup.exists():
            if config.MARKER in current:
                raise RuntimeError(f"[remnant] missing original backup: {path}")
            if restoring:
                continue  # Nothing owned by Remnant to restore; never use git checkout.
        original = backup.read_text() if backup.exists() else current
        if config.MARKER in original:
            raise RuntimeError(
                f"[remnant] backup is not an unpatched original: {backup}"
            )
        ...
        if current not in (original, expected):
            raise RuntimeError(
                f"[remnant] unexpected edits or incompatible patch in {path}; "
    ...
    try:
        for path, current, original, expected in plan:
            if path.read_text() != current:
                raise RuntimeError(
                    f"[remnant] source changed during patch operation: {path}"
                )
            target = original if restoring else expected
            if current == target:
                continue
            backup = Path(str(path) + ".remnant.orig")
        ...
        raise
    for path, *_ in plan:
        print(f"[remnant] {path}: {'restored' if restoring else 'patched/verified'}")
...
def verify() -> None:
    ...
    for path, current, _, expected in plan:
        if current != expected:
            raise RuntimeError(f"[remnant] missing or incomplete patch: {path}")
    for path, current, *_ in plan:
        print(f"{path}: remnant_markers={current.count(config.MARKER)}")
...
def drift() -> int:
    ...
    .remnant.orig backup when the tree is already patched, else the file as it
    ...
    for filename, factory in _TARGETS:
        path = Path(filename)
        status = "patched" if Path(str(path) + ".remnant.orig").exists() else "pristine"
        if not path.exists():
