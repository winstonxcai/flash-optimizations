...
def _backend_edits(source: str | None = None):
    ...
    if prefill_anchor not in current_backend:
        if v17_prefill_anchor not in current_backend:
            raise AssertionError(
                "[remnant] unsupported DSV4 sparse-prefill anchor; "
    ...
    return [
        ...
        (
            ...
            "            ):\n"
            "                # NOTE: enable_hierarchical_cache is intentionally NOT "
            "blocked here -- the\n"
            "                # remnant patch to hybrid_pool_assembler.py installs a "
            "packed-aware c4\n"
            "                # host mirror for packed+HiCache (patches/hicache.py). "
            "CPU/disaggregation\n"
            "                # offload still route through the native c4 ABI and "
            "stay incompatible.\n"
        ...
        (
            ...
            "            if (\n"
            "                compress_ratio == 4\n"
            "                and _sg_lr.sparse_enabled()\n"
            "                and not _is_sm120\n"
            "                and q.shape[1] == 1\n"
            "            ):\n"
            "                ## REMNANT (direct 328-byte c4 leg, no reassembly)\n"
            "                # v1 reads packed records only for single-token decode.\n"
            "                # A multi-token extend keeps the packed reconstruct path\n"
            "                # below, which is the already-validated one, and sm120 has\n"
            "                # no direct-read kernel at all.\n"
            "                _sg_physical = (\n"
            "                    extra_indices.squeeze(1)\n"
            "                    if extra_indices.ndim == 3 else extra_indices\n"
            "                )\n"
            "                _sg_raw = (\n"
            "                    raw_indices if raw_indices.ndim == 2\n"
            "                    else raw_indices.squeeze(1)\n"
            "                )\n"
            "                from sgl_kernel.flash_mla import (\n"
            "                    flash_mla_with_kvcache as _sg_mla,\n"
            "                )\n"
            "                # The SWA leg is unchanged native flash_mla with no c4\n"
            "                # extra cache -- attn_sink belongs to this leg alone, so\n"
            "                # the sink is counted exactly once across the split.\n"
            "                _sg_swa_out, _sg_swa_lse = _sg_mla(\n"
            "                    q=q,\n"
            "                    k_cache=swa_k_cache,\n"
            ...
            "                    swa_leg=lambda: (\n"
            "                        _sg_swa_out.squeeze(1),\n"
            "                        _sg_swa_lse.squeeze(-1),\n"
            "                    ),\n"
            "                )\n"
            "\n"
            "            if compress_ratio == 4 and _sg_lr.packed_enabled():\n"
