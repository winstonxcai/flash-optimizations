...
instead feeds the packed record straight to ``remnant._sparse``, which
...
def _load():
    ...
    with _load_lock:
        ...
        try:
            _extension = importlib.import_module("remnant._sparse")
        except Exception as exc:
            _load_error = exc
            raise RuntimeError(
                "Sparse MLA was requested, but remnant._sparse could not "
                "be loaded"
            ) from exc
    return _extension
