...
Extension: `remnant._sparse` · gate: `SGLANG_OPT_TOPMAG_SPARSE=1`
...
wired together by `remnant.sparse.sparse_forward`, which owns the softmax, its
...
Built as its own `CUDAExtension`, separate from `remnant._fused`, so a failure
