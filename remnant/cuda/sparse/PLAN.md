...
`remnant/` stores the DSV4-Flash CSA latent C4 state packed — TopMag50 keeps 256
...
[remnant/cuda/fused.cu](../fused/fused.cu)
(`packed_to_native_kernel`, `remnant._fused`), **reassembles** each packed row
...
[remnant/](../../), `main` @ `86fa14d`) is a Flash-LLM-derived
...
kernel launches (`remnant_key_formulation` → softmax → `remnant_value_formulation`,
[models/llama_remnant_kernel.py:274,314](../../models/llama_remnant_kernel.py#L274)).
...
### Vendored tree — `remnant/cuda/sparse/`
...
`bindings.cpp` (pybind module `remnant._sparse`),
`remnant/sparse.py` (loader, mirroring [fused.py](../../fused.py)),
and a second `CUDAExtension` in [cuda/setup.py](../setup.py)
so `remnant._fused` and `remnant._sparse` build independently.
...
`remnant/sparse.py` exposes `sparse_forward(...)`, doing the whole
...
- **CPU:** `python -m unittest remnant.tests.test_patching` offline (passes
...
- Landing the vendored tree into the outer repo's gitignore rules — `remnant/`
