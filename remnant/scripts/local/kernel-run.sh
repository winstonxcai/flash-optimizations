#!/usr/bin/env bash
# =====================================================================
# kernel-run.sh -- run the remnant GPU suites in a container pinned to a
#               GPUQ-granted device.
#
#   gpuq run --project remnant --gpus 1 --timeout 30m \
#     --cwd /home/jovyan/winstonxcai/flash-optimizations \
#     -- bash remnant/scripts/local/kernel-run.sh
#
#   gpuq lease acquire --project remnant --gpus 1 --gpu-ids 5 \
#     --ttl 45m --keep-idle --detach          # -> lease-...
#   REMNANT_DEVICES=5 REMNANT_LEASE=lease-... \
#     bash remnant/scripts/local/kernel-run.sh  # the container is the CUDA process
#
...
#   2. REMNANT_DEVICES plus REMNANT_LEASE, when a container has to be launched
...
# are also exported as REMNANT_RESULTS_DIR for speed.py's speed.{json,csv}.
# =====================================================================
set -euo pipefail
...
if [ -n "${GPUQ_JOB_ID:-}" ]; then
  ...
  [ -n "${REMNANT_DEVICES:-}" ] || die "no allocation: run this under 'gpuq run', or set REMNANT_DEVICES (and REMNANT_LEASE) from a lease you hold"
  [ -n "${REMNANT_LEASE:-}" ] || die "REMNANT_DEVICES=${REMNANT_DEVICES} names no lease; set REMNANT_LEASE to the lease that granted it"
  CONTEXT="lease ${REMNANT_LEASE}"
  DEVICES=$REMNANT_DEVICES
fi
...
NAME=remnant-kernel-run
RUN_TIMEOUT=${RUN_TIMEOUT:-1800}

echo "== ${CONTEXT}  project=${GPUQ_PROJECT:-remnant}  devices=${DEVICES}"
...
if [ "$MODE" != "--speed-only" ]; then
  INNER="$INNER
python3 -m remnant.tests.validity 2>&1 | tee $OUT/validity.log"
fi
if [ "$MODE" != "--validity-only" ]; then
  INNER="$INNER
python3 -m remnant.tests.speed 2>&1 | tee $OUT/speed.log"
fi
